...
var styleContent = __webpack_require__(/*! ./utils/template-style */ "./src/utils/template-style.js");

var combineTemplate = __webpack_require__(/*! ./utils/index */ "./src/utils/index.js");

var compile = __webpack_require__(/*! ./utils/compiler */ "./src/utils/compiler.js");

var template = __webpack_require__(/*! lodash/template */ "./node_modules/lodash/template.js");

var chunk = __webpack_require__(/*! lodash/chunk */ "./node_modules/lodash/chunk.js");
...
function processData(temp, data) {
  ...
  temp.filter(function (_ref) {
    ...
    if (item.type === "layout") {
      ...
      var itemsEntries = items.map(function (item) {
        return Object.entries(item);
      });
      itemsEntries = itemsEntries.map(function (item) {
        return adjustSequence(item, sequence).map(function (_ref2) {
      ...
      var _itemsEntries = data[item.name].map(function (item) {
        return Object.values(item);
      });

      ret[item.name] = _itemsEntries;
...
function printReciept(template$, data$) {
  ...
  var temp = combineTemplate(temp$);
  var data = processData(temp$, data$);
  var printIframe = document.createElement("iframe");
  var meta = document.createElement("meta");
  meta.setAttribute("http-equiv", "Content-Type");
  meta.setAttribute("content", "text/html; charset=utf-8"); //设置打印样式

  var printStyle = document.createElement("style");
  printStyle.type = 'text/css';
  printStyle.innerHTML = styleContent; //设置iframe样式
  ...
  document.body.append(printIframe); //置样式
...
var basicTemplate = function basicTemplate(name) {
  return "<div class=\"reciept-wrapper\">\n<% \n    ".concat(name, ".forEach( function (item) { %>\n        <div class=\"outer\">\n            <%if(!Array.isArray(item[0])){%>\n                <%= item[0] %>:<%= item[1] %>\n            <%}else{item.forEach(function(element){%>\n               <div class=\"inner\"> <%= element[0] %>:<%= element[1] %></div>\n            <% })}%>\n        </div>\n    <% });\n%>\n</div>");
}; //<div class="inner<%if(i===1){%>3<%}else if(i===4){%>2<%}%>"><%= element %></div>


var itemsTemplate = function itemsTemplate(name) {
  return " <div class=\"reciept-wrapper\">\n<div class=\"table-header\">\n    <div class=\"table-outer\">\n      <% ".concat(name, ".headers.forEach( function (item,i) { %>            \n           <div class=\"inner<%if(!i){%><%}%>\"> <%= item %></div>\n        <% })%>    \n      </div>\n</div>\n<div class=\"low-height\"><br/><br/></div>\n<div class=\"table-content\">\n    <% ").concat(name, ".items.forEach(function(item){ %>   \n        <div style=\"word-break:break-all\"><%=item[0]%></div>     \n        <div class=\"table-outer\">        \n        <% item.forEach(function(element,i){if(!!i){%>\n            <%=element%>\n        <%} }) %>        \n       </div>       \n       <div class=\"low-height\"><br/></div>\n    <% })  %>\n</div>\n</div>");
};

var betweenItemsTemplate = function betweenItemsTemplate(name) {
  return " <div class=\"reciept-wrapper\">\n\n<div class=\"table-content\">\n    <% ".concat(name, ".forEach(function(item){ %>        \n        <div class=\"table-outer-between\">\n        <% item.forEach(function(element,i){%>\n            <div><%= element %></div>\n        <%}) %>        \n       </div>       \n       <div class=\"low-height\"><br/></div>\n    <% })  %>\n</div>\n</div>");
}; //拼装模板


var combineTemplate = function combineTemplate(arr) {
  var concatStr = function concatStr(str, count) {
    return new Array(count).fill(str).join("");
  };

  var typeToTemplatesMapping = function typeToTemplatesMapping(type) {
    // basic:basicTemplate,
    ...
    if (/^br(\d?)$/.test(type)) {
      var _type$match = type.match(/^br(\d?)$/),
          _type$match2 = _slicedToArray(_type$match, 2),
          _ = _type$match2[0],
          count = _type$match2[1];

      count = count || 1;
      return function () {
        return "<div class=\"low-height\">".concat(concatStr("<br/><br/>", Number(count)), "</div>");
      };
    }

    if (/^line(\d?)$/.test(type)) {
      var _type$match3 = type.match(/^line(\d?)$/),
          _type$match4 = _slicedToArray(_type$match3, 2),
          _2 = _type$match4[0],
          _count = _type$match4[1];
      ...
      return function () {
        ...
        return "<div class=\"low-height\"><div class=\"line\"></div>".concat(concatStr("<br/><div class=\"line\"></div>", _count - 1), "</div>");
      };
    }

    if (/^title$/.test(type)) {
      return function (title) {
        return "<div class=\"title\"><%=".concat(title, "%></div>");
      };
    }
